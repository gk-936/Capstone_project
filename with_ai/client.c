...
int main() {
    ...
    while (1) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (strlen(message) > 0) {
                // If it's an ask command, send it as is (server will handle it)
                if (strncmp(message, "\\ask", 4) == 0) {
                    ...
                    // For regular messages, add the username
        ...
        if (fds[1].revents & POLLIN) {
            ...
            if (strncmp(buf, "AI|", 3) == 0) {
                ...
                printf("\n[AI] %s\n", buf + 3);  // Skip the "AI|" prefix
            } else if (strncmp(buf, "MSG|", 4) == 0) {
                // This is a regular chat message
                printf("\n[Server] %s\n", buf + 4);  // Skip the "MSG|" prefix
            } else {
                // For backward compatibility, display messages without prefix as is
                printf("\n[Server] %s\n", buf);
            }
        }
    }
